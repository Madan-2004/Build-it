...
const ClubMembers = ({ 
  ...
  return (
    <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-xl overflow-hidden`}>
      <div className={`px-6 py-4 flex justify-between items-center ${darkMode ? 'bg-gradient-to-r from-blue-800 to-indigo-900' : 'bg-gradient-to-r from-blue-500 to-indigo-500'}`}>
      ...
      {club.members && club.members.length > 0 ? (
                ...
                {memberEditMode === member.user.id ? (
                  ...
                  <div>
                    ...
                    <div className="mt-2">
                      ...
                      {member.joinDate && (
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${darkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-200 text-gray-700'} ml-2`}>
        ...
        <div className={`p-8 text-center ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
          ...
          <p className="text-xl font-medium mb-2">No members have been added yet.</p>
          {isAdmin && (
            <p className="text-md">Click the "Add" button to add members to this club.</p>
          )}
        </div>
      )}
    </div>
