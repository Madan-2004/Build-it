...
class ElectionViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
                # Save the election instance
        election = serializer.save(created_by=self.request.user)

        # Handle the voter file if provided
        voter_file = self.request.FILES.get('voter_file')
        if voter_file:
            VoterFile.objects.create(file=voter_file, election=election)
    ...
    def update(self, request, *args, **kwargs):
        ...
        # Handle voter file updates
        voter_file = request.FILES.get('voter_file')
        if voter_file:
            # Delete the old voter file if it exists
            instance.voter_files.all().delete()
            # Create a new voter file
            VoterFile.objects.create(file=voter_file, election=instance)
